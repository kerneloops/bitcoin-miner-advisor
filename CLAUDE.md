...
| 25 | Prediction market signals (Kalshi / Polymarket) | Research and integrate prediction market APIs as a forward-looking signal source. Candidates: Kalshi (CFTC-regulated, REST API, contracts on Fed rate decisions, recession probability, inflation, BTC price targets), Polymarket (crypto-native, covers crypto regulatory events, BTC milestones). Signal value: prediction markets are crowd-sourced implied probabilities — complementary to technicals (backward-looking) and options IV (market-maker-driven). Most actionable contracts: Fed rate cut/hike probability (directly moves risk assets), BTC year-end price brackets, crypto regulation outcomes. Research phase: evaluate API availability, free tier limits, data freshness, cost, and legal considerations before building. Implementation: fetch relevant contract probabilities, cache in macro_signals table, surface in macro panel and pass to Advisor context. |
